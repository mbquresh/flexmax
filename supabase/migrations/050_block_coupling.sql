-- 050_block_coupling.sql
-- Applied manually via Supabase SQL Editor. Do not run `supabase db push`.
--
-- Rewritten from 049_evidence_baseline.sql. Every existing section is
-- untouched except cannibalization, which is replaced by block_coupling.
-- Adds couple_base (60-day), keystones, day_of_week, and a coupling_note
-- caveat. tracked / engaged / the 30-day base CTE are unchanged.
--
-- Thresholds (do not lower): recent days >= 10, n_won >= 6, n_lost >= 6,
-- abs(lift) >= 30. Persistence across the prior 30 days is the
...
-- );
-- select jsonb_pretty(
--   (public.get_behavior_evidence('YOUR-USER-ID')) -> 'day_of_week'
-- );
--
-- If block_coupling is empty, do not lower the thresholds. Inspect recent
-- agg instead (same pairing, no filter). Paste YOUR-USER-ID and run:
--
-- with v as (
--   select (now() at time zone p.timezone)::date as v_today
--   from profiles p where p.id = 'YOUR-USER-ID'
-- ),
-- tracked as (
--   select i.block_id
--   from daily_schedule_instances i, v
--   where i.user_id = 'YOUR-USER-ID'
--     and i.date >= v.v_today - 30
--     and i.date < v.v_today
--   group by i.block_id
--   having count(*) filter (where i.status in ('completed','missed'))
...
--   cross join v
-- )
-- select trigger_name, later_name, recent,
--        count(*) as days,
--        count(*) filter (where trigger_won) as n_won,
--        count(*) filter (where not trigger_won) as n_lost,
--        round(100.0 * count(*) filter (where trigger_won and later_failed)
...
--         - round(100.0 * count(*) filter (where not trigger_won and later_failed)
--               / nullif(count(*) filter (where not trigger_won), 0))::int) as lift
-- from pairs
-- where recent
-- group by 1, 2, 3
-- order by abs(
--   coalesce(round(100.0 * count(*) filter (where trigger_won and later_failed)
--                  / nullif(count(*) filter (where trigger_won), 0))::int, 0)
--   - coalesce(round(100.0 * count(*) filter (where not trigger_won and later_failed)
--                  / nullif(count(*) filter (where not trigger_won), 0))::int, 0)
-- ) desc;
