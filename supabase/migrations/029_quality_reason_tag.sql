-- 029_quality_reason_tag.sql
-- Applied manually via Supabase SQL Editor.
--
-- Adds quality_reason_tag: why a block that is still COMPLETING is degrading.
-- Captured at the check-in that crosses the drift threshold (4 of the last 7
-- rated instances rated partial/pulled_away). This is the earliest warning
-- available -- it fires before the block starts being missed at all.
--
-- Deliberately a NEW column, not a reuse:
--   * miss_reason_tag is nulled on completion and aggregates across MISSED
--     blocks. Writing completion reasons into it would corrupt miss_reasons.
--   * reflection_why is free text. Tapped presets written there would be
--     quoted back as the user's own words -- the exact error corrected in the
--     reflection_improve removal.
--
-- Also widens quality_drift's recent window from 5 to 7 rated instances so the
-- evidence pack and the in-app prompt share one definition of "degrading".
-- Without this the app would ask about a block the weekly insight never
-- mentions. NOTE: this MOVES recent_poor / recent_rated, which the narrator
-- already reads. Expect different numbers, not a different meaning.

alter table daily_schedule_instances
  add column if not exists quality_reason_tag text;

-- NOT VALID, matching 025: existing rows are not checked so this cannot fail
-- on legacy data, but it governs everything written from here on.
alter table daily_schedule_instances
  drop constraint if exists quality_reason_tag_valid;

alter table daily_schedule_instances
  add constraint quality_reason_tag_valid check (
    quality_reason_tag is null or quality_reason_tag in (
      'Interrupted',
      'Low energy',
      'Started late',
      'Wrong time of day',
