...
Edge functions (3):
  weekly-insight   — 1 AI call per user on Mon/Wed/Fri (local); the only AI call in the product. Off-slot days and a same-day retry are cache hits. A set older than 7 days always regenerates.
...
Pipeline:
  get_behavior_evidence(user_id)  [SQL, 013 / 054 — does ALL arithmetic,
    ...
    → weekly-insight edge function [1 AI call per user on Mon/Wed/Fri]
...
| Theory of You | app/you.tsx + DisputeSheet + 047. Menu and title are "Theory of You". Twelve-week two-tone chart, 30-day accounted/landed as one caption, then the current set grouped by kind: What's holding (green / strength), Check engine (coral engine mark / structural), Worth watching (yellow / pattern, up to 3 cards), What's slipping (red / causal). An "As of {date}" caption is the current set's generated_at. Belief then muted evidence. Tap a line: "That's not right" → one note → the line leaves immediately. dispute_insight writes insight_corrections (survives supersede) and stamps disputed_at. Morning InsightCard and recovery omit disputed rows. weekly-insight reads the last 20 corrections and must not restate a rejected belief. No second AI call on the tap. No Done button — the X is enough. `kind` may be `structural` (052); InsightCard still hides only `strength`, and a structural morning card wears the same coral engine mark. |
...
2400` in `weekly-insight`), on Mon/Wed/Fri, gated behind `engaged_days >= 5`.
