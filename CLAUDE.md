...
| 046 | block_time_overrides.sql | Sparse per-weekday time overrides on schedule_blocks. Base columns unchanged, nothing backfilled. Live column is NOT NULL — client writes `{}`, never null |
| 047 | generate_time_overrides.sql | Both generate_* functions resolve time_overrides for that dow; DEFAULT '{}' on the column so an omitted insert is not a 23502 |
