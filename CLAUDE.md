...
- Two GestureDetectors, not Gesture.Race: drag on the handle rail,
...
Edge functions (3):
  weekly-insight   — 1 AI call per user on Mon/Fri (local); the only AI call in the product. Off-slot days and a same-day retry are cache hits. A set older than 7 days always regenerates.
...
Pipeline:
  get_behavior_evidence(user_id)  [SQL, 013 / 054 — does ALL arithmetic,
    ...
    → weekly-insight edge function [1 AI call per user on Mon/Fri]
...
| What                                          | Where                                                 |
| --------------------------------------------- | ----------------------------------------------------- |
| Rate limiting on all AI edge functions        | calendar-feed only. weekly-insight uncapped while the narrator is being refined. |
...
| Theory of You | app/you.tsx + DisputeSheet + 047. Menu and title are "Theory of You". Twelve-week two-tone chart, 30-day accounted/landed as one caption, then the current set grouped by kind: What's holding (green / strength), Check engine (coral engine mark / structural, one card, two only when a second qualified finding sits at the same level), Worth watching (yellow / pattern, up to 3 cards), What's slipping (red / causal). An "As of {date}" caption is the current set's generated_at. Belief then muted evidence. Tap a line: "That's not right" → one note → the line leaves immediately. dispute_insight writes insight_corrections (survives supersede) and stamps disputed_at. Morning InsightCard is an OBD-style "Report ready" strip — counts only, no belief text; tap opens /you. Recovery omit disputed rows. weekly-insight reads the last 20 corrections and must not restate a rejected belief. No second AI call on the tap. No Done button — the X is enough. `kind` may be `structural` (052). |
...
| Discriminating tests | 054 + weekly-insight rule 10. SQL runs three fixed tests (anchor control, domain spread, gap sensitivity) on qualified pairs and emits `surviving` / `ruled_out` / `untested` for carry / upstream / cascade. Narrator maps those lists to spoken English — never test names, never lift, never "anchor control". Anchor emit includes anchored arm counts so the copy can say "0 of N" instead of "−58 points". Belief 400 / evidence 480. A leak detector drops internals before write, including completed_7d / failed_7d / completed_prior / failed_prior — speak the counts, never the keys. Gap will usually be `insufficient_data` — that is correct. Paste 054, then redeploy weekly-insight. |
...
2400` in `weekly-insight`), on Mon/Fri, gated behind `engaged_days >= 5`.
Off-slot days are a cache hit. weekly-insight is uncapped while the narrator
is being refined. A free user who churns in week one triggers zero calls.
